...
public class JunctionTable {
	...
	private class JunctionRecord extends Record {
		...
		public JunctionRecord(String tableName, boolean hasDates,
				HashMap<String, Field> record) {
			super(tableName, hasDates);
			this.initializeFields(record);
		}
	}
	
	// ArrayList of field arrays that are at least 3 elements long-pkey,
	// record1_fk, record2_fk
	...
	public JunctionTable(String table,
			ArrayList<HashMap<String, Field>> junctionRecords, String type1,
			String type2, boolean hasDates) {
	...
	public void saveRecords() throws SQLException {
		for (HashMap<String, Field> element : this.elements) {
			JunctionRecord record = new JunctionRecord(this.tableName,
					this.hasDates, element);
			record.save();
		}
	}
}
