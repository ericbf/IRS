...
public abstract class ListView<E extends Record> extends View {
    ...
    protected String[] buttonNames = {"Add", "Edit", "Toggle Active", "View", "Generate Nonfinal Invoice(s)", "Close and Generate Invoice"};
    ...
    public ListView(SplitPane splitPane) {
        ...
        this.searchField.addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                boolean meta = (e.getModifiers() & ActionEvent.META_MASK) == ActionEvent.META_MASK || (e.getModifiers() & ActionEvent.ALT_MASK) == ActionEvent.ALT_MASK;
                // System.out.println(e.getKeyText(e.getKeyCode()) + " "
                // + e.getKeyCode() + " " + e.getExtendedKeyCode());
                switch (e.getKeyCode()) {
                    case BACKSPACE:
                        if (meta) {
                            int caretPosition = this.searchField.getCaretPosition();
                            if (caretPosition < this.searchField.getText().length()) {
                                this.searchField.setText(this.searchField.getText().substring(caretPosition));
                    ...
                    case DOWN:
                        if (ListView.this.table.getRowCount() > 0) {
                            ...
                            if (meta) {
                                ...
                                ListView.this.table.setRowSelectionInterval(downSel, downSel);
                            } else if (downSel != -1) {
                                downSel += downSel + 1 < ListView.this.table.getRowCount() ? 1 : 0;
                                ListView.this.table.setRowSelectionInterval(downSel, downSel);
                            } else {
                                ListView.this.table.setRowSelectionInterval(0, 0);
                            }
                        }
                        break;
                    case UP:
                        if (ListView.this.table.getRowCount() > 0) {
                            ...
                            if (meta) {
                                ...
                                upSel -= upSel > 0 ? 1 : 0;
                                ListView.this.table.setRowSelectionInterval(upSel, upSel);
                            } else {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                switch (e.getKeyCode()) {
                    case DOWN:
                        if (ListView.this.table.getRowCount() > 0) {
                            if (meta) {
                                int rowCount = ListView.this.table.getRowCount() - 1;
                                ListView.this.table.setRowSelectionInterval(rowCount, rowCount);
    ...
    public class HintField extends JTextField {
        ...
        public HintField(String hint) {
            ...
            this.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, java.awt.event.InputEvent.META_DOWN_MASK), "none");
            this.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, java.awt.event.InputEvent.META_DOWN_MASK), "none");
